  ...
  return async function (req, res, next) {
    ...
    let map={};
        ...
        */
        ...
        console.log('This is response req.ret_data status ' + req.ret_data.status);
        ...
        if(req.ret_data.status){
            if(req.ret_data.data.permission=="" || req.ret_data.data.permission=="no" || req.ret_data.data.permission==false){
                ...
                console.log('this is the role id ' + response_data.user_detail.RoleId);
                ...
                let department = await models.Department.findOrCreate({where: {dept_name: department_name, CurrentBusinessId: currentBusiness[0].id}});
                let role = await models.Role.findOrCreate({where: {role_name:  role_name, CurrentBusinessId: currentBusiness[0].id}});
